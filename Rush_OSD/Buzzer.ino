...
void buzzer(uint8_t warn_vbat){
  ...
  if ((MwSensorActive&GPSHOLDMODE || MwSensorActive&GPSHOMEMODE) && !GPS_fix){    //if no fix and gps funtion is activated: do warning beeps.
    warn_noGPSfix = 1;    
  }
  else{
    warn_noGPSfix = 0;
  ...
  if (armedTimer >= (armedtimeWarning*60)){
    warn_runtime = 1;
  }
  ...
  else if (warn_vbat == 1)     beep_code('S','M','M','D');       // Low voltage alarm...
  else if (warn_vbat == 2)     beep_code('S','M','N','D');       // High temperature alarm...

  else if (warn_runtime == 1 && armed == 1)beep_code('S','S','M','N'); //Runtime warning            
